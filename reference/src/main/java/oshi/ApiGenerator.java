 ...
 * {@code definitions.json} file in the root project directory.
 * 
 * @author cilki
 * @since 5.0.0
 */
public final class ApiGenerator {
	...
	private static class ComponentJson {
		...
		public String getPkgName() {
			return "oshi.api." + name.substring(0, name.lastIndexOf('.')) + "." + getBaseName().toLowerCase();
		}
	}
	...
	private static void generateAttributeEnums(ComponentJson component) throws Exception {
	...
	private static void generate(ComponentJson component, String platform) throws Exception {
		String baseName = component.getBaseName();
		String pkgName = component.getPkgName();
		...
		if (!platform.isEmpty()) {
			container.superclass(ClassName.get(pkgName, baseName + "Container"));
		...
		container.addField(FieldSpec
				.builder(ClassName.get("oshi.driver", "ComponentDriver"), "driver", PRIVATE, TRANSIENT).build());

		// Create attach method
		var attach = MethodSpec.methodBuilder("attach")
				.addParameter(ClassName.get("oshi.driver", "ComponentDriver"), "driver")
		...
		files.add(JavaFile.builder(pkgName, container.build()).addFileComment(GENERATOR_COMMENT).build());
	...
	private static void generateSystemInterface(List<ComponentJson> components, String platform) {
		...
		files.add(JavaFile.builder("oshi.api", system.build()).addFileComment(GENERATOR_COMMENT).build());
	}
